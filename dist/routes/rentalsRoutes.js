...
router.get("/", [auth, admin], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    res.send(yield Rental.find().populate(["book", "user"]).sort("-_id"));
...
router.get("/:id", [validateObjectId, auth, admin], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const rental = yield Rental.findById(req.params.id).populate([
        "book",
        "user",
    ]);
    if (!rental)
...
router.post("/", [auth, admin, validateBody(rentalSchemaObject)], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const book = yield Book.findById(req.body.book);
    if (!book)
        return res.status(400).send("Invalid Book.");
    if (book.numberInStock === 0)
        return res.status(400).send(`'${book.title}' is out of stock`);
    const session = yield conn.startSession();
    try {
        yield session.withTransaction(() => __awaiter(void 0, void 0, void 0, function* () {
            const rental = yield new Rental({
                user: user._id,
                book: book._id,
            }).save();
            yield Book.findByIdAndUpdate(book._id, {
            ...
            res.status(201).send(yield Rental.findById(rental._id).populate(["book", "user"]));
        }));
        session.endSession();
    }
    catch (ex) {
...
//     res.send(rental);
// });
router.patch("/return/:id", [validateObjectId, auth, admin], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const rentalInDb = yield Rental.findById(req.params.id);
    if (!rentalInDb)
        return res.status(404).send("Resource not found");
    ...
    try {
        yield session.withTransaction(() => __awaiter(void 0, void 0, void 0, function* () {
            const user = yield User.findByIdAndUpdate(rentalInDb.user, {
                $pull: { activeRentals: req.params.id },
            }, { runValidators: true });
            const book = yield Book.findByIdAndUpdate(rentalInDb.book, {
                $inc: { numberInStock: 1, numberRentedOut: -1 },
            }, { runValidators: true });
            rental = yield Rental.findByIdAndUpdate(req.params.id, { $set: { dateReturned: new Date() } }, { new: true, runValidators: true }).populate(["book", "user"]);
        }));
        session.endSession();
    }
    catch (ex) {
